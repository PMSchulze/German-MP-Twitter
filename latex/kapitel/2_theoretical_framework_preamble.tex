...
\vspace{-0.25cm}
\item[•] A \textit{topic-word distribution} \textbf{\beta} is a probability distribution over words, i.e., 
\vspace{-0.25cm}
\item[•] A \textit{topic assignment} \textbf{z_{d,n}} is the assignment of the $n$-th word of document $d$ to a specific topic $k \in \{1,\dots,K\}$ (i.e., to the corresponding word distribution \textbf{\beta_k}). Therefore, \textbf{z_{d,n}} is simply a vector of length $K$ whose $k$-th entry equals one and all other entries equal zero. This way, we can represent the word distribution corresponding to the $n$-th word in document $d$ as $\beta_{d,n}:=Bz_{d,n}$ (again, for a model without topical content variable).
\vspace{-0.25cm}
