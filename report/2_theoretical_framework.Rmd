...
The graphical model representation in Figure 1 visually describes the detailed mechanism underlying the STM. The generative process of the model is as follows:
...
3) For each word $n \in \{1,\dots,N_d\}$:
    a) Draw a topic assignment $z_{d,n} \sim Multinomial_K(\theta_d)$.
    b) If no topical content variable has been specified, simply draw a word $w_{d,n} \sim Multinomial_V(\beta_{d,n})$. Otherwise, first determine the document-specific word distributions $\beta_{k,a}$ based on the level $a$ taken on by $Y_d$, for all topics $k \in \{1,\dots,K\}$: $B_a := [\beta_{1,a}|\dots|\beta_{K,a}]$; next, draw a word $w_{d,n} \sim Multinomial_V(B_az_{d,n})$.
...
1 page high-level + ELBO in section 2
1 page more technical in appendix
