...
In this section, we describe how inference and parameter estimation for topic models, in particular for the STM, are performed. As a detailed discussion of the underlying workings is outside of the scope of this paper, we refer the reader to the appendix and the referenced papers.
...
Altogether, the STM uses an Expectation-Maximization (EM) algorithm. In the E-step, the variational posterior distributions for topic proportions, $q(\theta_d)$, and for topic assignment, $q(z_{d,n}$, are updated, using Laplace variational inference and coordinate ascent. In the M-step, the model parameters - specifically topical prevalence and content coefficients - are updated by maximizing *ELBO* with respect to them (@roberts2016model).


## Appendix: Inference and Parameter Estimation

Wang and Bleib p.1007

The exact posterior distribution is intractable to compute due to the (high-dimensional) marginal distributions in the denominator, which is why exact inference is infeasible and variational inference is used instead. Variational inference involves approximating the posterior by postulating a simple distribution family for the (joint) distribution of latent model variables $\theta$ and $z$ - $q(\theta,z)$ - and subsequently determining the member of this family which minimizes the "distance" to the true posterior distribution, measured using the Kullback-Leibler (KL) divergence. Note that, for computational purposes, we compute KL divergence of the true posterior $p$ from the approximating posterior $q$, $KL(q||p)$, whereas intuitively we would think $KL(p||q)$.

The most popular variational inference technique is mean-field variational inference (also: mean-field variational Bayes), where we posit full factorizability of $q(\theta,z)$, i.e., $q(\theta,z) = q(\theta)q(z)$; that is, $\theta$ and $z$ are assumed to be independent with their own distributions and variational parameters (which we suppress for improved readability). Note that the posited approximate distribution family, while flexible, does *not* contain the true posterior distribution, because $\theta$ and $z$ are actually dependent. 

Let us now write out the KL divergence, where $\theta$ and $z$ are latent variables and data $x$ is observable:
...
The left-hand side of the above inequality is the marginal log likelihood of observed data x and is also called evidence (of the observed data). Note that the evidence is not computable - otherwise we would not need to use variational inference in the first place. The right-hand side thus presents a lower bound on the evidence and we define the *Evidence Lower BOund* (ELBO) as: 

\begin{align*}
ELBO := \mathbb{E}_q[log(p(\theta,z,x))] - \mathbb{E}_q[log(q(\theta,z))],
\end{align*}

where the second component of the ELBO, $\mathbb{E}_q[log(q(\theta,z))$, is the entropy of the approximate distribution $q$. Equivalently, the evidence constitutes an upper bound for the ELBO.

This means that we actively maximize the ELBO (which is therefore also called *variational objective*), which in turn is equivalent to minimizing the KL divergence between the approximate distribution $q(\theta,z)$ and the true posterior $p(\theta,z|x)$. Therefore, the approximation $q(\theta,z)$ - or, more precisely, the variational parameters of $q(\theta)$ and $q(z)$ - that maximizes the ELBO simultaneously minimizes KL divergence (@blei2003latent; @wang2013variational).

Wang and Blei p.1008: optimality conditions

p.1011: section 3.1.2 first couple of lines

Roberts: here eta instead of theta

finding
the maximum a posteriori (MAP) estimate ˆηd and approximating
the posterior with a quadratic Taylor expansion. This

also: laplace for z

for M-step: roberts, roberts appendix, LDA A.4
