...
packages_required <- c(
  "ggcorrplot", "gridExtra", "huge", "knitr", "scales", "stm", "stringi", "tidyverse", "tm"
)
...
The second most representative document, still for topic `r topic_number`, with a proportion of `r scales::percent(repr_docs[topic_number_long][2,1], accuracy = 0.01)` is the one by MP `r repr_docs$Name[2]`, a member of the `r repr_docs$Partei[2]` party from `r repr_docs$Bundesland[2]`, from `r repr_docs$Datum[2]` which starts with:
...
plot.topicCorr(mod_prev_corr, vlabels = topic_labels)
```
