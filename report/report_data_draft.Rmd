...
packages_required <- c(
  "betareg", "grid", "gridExtra", "huge", "knitr", "mvtnorm", "quanteda", "reshape2", "scales", "stm", "stringi", "tidyverse", "tm"
)
...
We can now apply the method of composition, based on a beta regression, in order to quantify covariate effects. Setting the number of simulations to `r nsims`, we thus draw `r nsims` times 
out of the (approximate) marginal distribution of $\xi^*$, $q(\xi | \Gamma, \Sigma, X)$. To finally obtain draws from the topic proportions, $\theta_{(k)}$, we simply input $\xi^*$ and the design matrix $\tilde{X}$ into the sigmoid function, which is the response function corresponding to beta regression. When visualizing the impact of a particular covariate, all other covariates are held at their median, in line with the methodology employed in the *stm* package.
