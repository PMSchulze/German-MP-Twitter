...
frequency_raw <- colMeans(mod_prev$theta[,1:K])
frequency_weighted <- colMeans((mod_prev$theta*weights/mean_weight)[, 1:K])

frequency_df <- data.frame(topic_labels, frequency_raw, frequency_weighted) %>% reshape2::melt(id = "topic_labels")
colnames(frequency_df) <- c("topic", "variable", "frequency")

ggplot(data = frequency_df, aes(x = topic, y = frequency, fill = variable)) + 
