...
make_median_xmat <- function(est_var, formula, metadata, isnum, range_est_var) {
  dat <- metadata[, -which(names(metadata) == est_var)]
  dat_new <- lapply(dat, function(x) if(is.numeric(x)) median(x) else majority(x))
  dat_fit <- data.frame(dat_new, fitvar = range_est_var)
...
predict_props <- function(beta_coefs, est_var, formula, 
  ...
  xmat <- stm::makeDesignMatrix(as.formula(paste("~",as.character(formula)[3])), metadata, dat_fit)
  res <- list()
  for (k in topic_nam){
    ...
    res[[k]] <- data.frame(dat_fit[[est_var]], mu, ci[1,], ci[2,])
...
## and full range of variable est_var, holding all variables but est_var as median/majority
sample_props_lognorm <- function(stmobj, est_var, formula, metadata, nsims = 1000, 
                                 ci_lower = 0.025, ci_upper = 0.975) {
  gamma <- stmobj$mu$gamma
  Sigma <- stmobj$sigma
  isnum <- is.numeric(metadata[,est_var])
  if(isnum) {
    range_est_var <- seq(min(metadata[,est_var]), max(metadata[,est_var]), length.out = 500)
  } else {
    range_est_var <- unique(metadata[,est_var])
  }
  xmat <- make_median_xmat(est_var, formula, metadata, isnum, range_est_var)
  mu <- xmat %*% gamma
